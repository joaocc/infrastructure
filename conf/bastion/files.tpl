...
- path: /opt/bin/github-keys
  ...
  content: |
    ...
    keys=`DOCKER_HOST=unix:///var/run/early-docker.sock docker run --net host --rm brandfolder/github-keys:latest --token $github_token brandfolder bastion user-keys $user`
    if [ $? -ne 0 ] ; then
