...
- path: /etc/profile.d/cluster-versions-alias.sh
  permissions: '0755'
  content: |
    alias cluster-versions='echo "" ; for ip in `fleetctl list-machines --no-legend --fields ip | grep bastion --invert-match` ; do fleetctl list-machines | grep $ip ; ssh -o "StrictHostKeyChecking no" $ip "sh -c \"cat /etc/os-release | grep VERSION=\"" ; echo "" ; done'
